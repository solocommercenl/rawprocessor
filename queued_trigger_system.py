...
class QueuedTriggerSystem:
    ...
    async def _handle_raw_change_queued(self, change: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Queueing raw {operation_type} for car_id={car_id}")
            
            # Queue processing jobs for all active sites
            ...
            logger.debug(f"Queued {len(job_ids)} processing jobs for car_id={car_id}")
